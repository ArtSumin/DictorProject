...
Мост между macOS-приложением (Этап 3) и Python STT сервером (Этап 1).
SwiftUI будет вызывать `dictor-cli` через `Process`, получать текст из stdout.

```
SwiftUI  →  Process("dictor-cli", ["audio.wav"])  →  stdout: "Распознанный текст"
```
